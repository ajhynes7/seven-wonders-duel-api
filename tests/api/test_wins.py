...
def test_get_wins(client: TestClient, games: list[Game]):
    ...
    response_total = client.get("/wins/total")

    assert response_total.status_code == 200
    assert response_total.json() == [
        {"player_id": 1, "count": 2},
        {"player_id": 2, "count": 1},
    ]
...
@pytest.mark.usefixtures("scores")
def test_get_wins_with_tie(session: Session, client: TestClient, games: list[Game]):
